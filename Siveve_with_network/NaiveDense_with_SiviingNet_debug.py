...
def Sieve(A, W, im, r, frac_bits=28, p=5):
...
class NaiveDense:
    ...
    def __call__(self, inputs, im=0, r=0): # ****待測試正確性****
        Sieve_result = Sieve(inputs, self.W_binary, im, r)
...
def one_training_step(model, images_batch, labels_batch, learning_rate=1e-4, im=0.1, r=0): # *new*
...
def binary_ternary_convert(model): # ****待測試正確性****
   for layer in model.layers:
...
def fit(model, images, labels, test_images, test_labels, epochs, batch_size):
    ...
    for epoch_counter in range(epochs): # 進行epochs次訓練
        ...
        batch_generator = BatchGenerator(images, labels, batch_size) # 把dataset分批成num_batches等分(num_batches = dataset / batch_size)
        for batch_counter in range(batch_generator.num_batches): # 進行num_batches次訓練
            ...
            binary_ternary_convert(model)
            loss = one_training_step(model, images_batch, labels_batch) # 一小batche完整訓練一次
            if batch_counter % 2 == 0: # 每2個batches才print一次現在的loss多少
                print(f"loss at batch {batch_counter}: {loss:.2f}") # 所以這邊會print的次數等於訓練 num_batches / 2
        accuracy = evaluate_model(model, test_images, test_labels) # 紀錄這一個epoch的正確率
        accuracy_list.append(accuracy)
        end_time = time.time() # 一個epoch結束訓練時間點
        elapsed_time = end_time - start_time # 一個epoch總訓練時間
